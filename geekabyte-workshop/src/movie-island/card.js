...
class Card extends Component {
  ...
  onShowMoreClick() {
    this.setState(prevState => ({ showMore:
      !prevState.showMore}))
  }
    ...
    return (
      <div className="card">
        ...
        <div className="card-detail">
          ...
          <div className="card-desc">{data.overview.substr(0, showMore ? CHAR_LIMIT : data.overview.length)}
          {data.overview.length > CHAR_LIMIT && <div className="show-more" onClick={this.onShowMoreClick}>{showMore ? "Show more" : "Show less"}</div>}</div>
